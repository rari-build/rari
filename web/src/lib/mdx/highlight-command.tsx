...
import { PACKAGE_NAME_REGEX, WHITESPACE_SPLIT_REGEX } from '@/lib/utils'
  ...
  return (
    <>
      {parts.map((part, index) => {
        ...
        if (index === 0)
          return (
            <span key={key} className="text-(--cmd-bin)">
              {part}
            </span>
          )
        if (['create', 'install', 'add', 'run', 'dev', 'build', 'start', 'task'].includes(part))
          return (
            <span key={key} className="text-(--cmd-verb)">
              {part}
            </span>
          )
        if (part.startsWith('-'))
          return (
            <span key={key} className="text-(--cmd-flag)">
              {part}
            </span>
          )
        if (PACKAGE_NAME_REGEX.test(part))
          return (
            <span key={key} className="text-(--cmd-pkg)">
              {part}
            </span>
          )

        return (
          <span key={key} className="text-fg-body">
            {part}
          </span>
        )
      })}
