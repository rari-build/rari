...
rari requires Node.js ≥ 24.21.0, but not for the runtime. Node.js powers the orchestration layer: the `rari` CLI and the build toolchain. When you run `npm start`, the Node.js CLI spawns the Rust binary and exits. The Rust binary handles all HTTP requests with its embedded V8 engine.
