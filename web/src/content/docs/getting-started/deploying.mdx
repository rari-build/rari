...
- `engines.node` is set to `>=24.21.0` (required by rari)
...
You can deploy rari to any platform that runs Node.js ≥ 24.21.0. The general steps:
...
<CodeBlock filename="Dockerfile" language="dockerfile">{`FROM node:24.21-slim
