...
<CodeBlock language="tsx">{`<Layout>          {/* src/app/layout.tsx */}
...
export default function UserLayout({ children, params, pathname }: LayoutProps<'/users/[id]'>) {
  return (
    <div>
...
After `rari dev` or `rari build`, rari writes `src/rari-routes.d.ts` from your route tree so path-based `PageProps` (as in the example above) types `params` from the matched route. Object generics like `PageProps` with an explicit params shape still work.
...
Optional catch-all params are `readonly string[] | undefined` — omitted (undefined) when the route matches with no extra segments.
