...
The `localPatterns` array defines which paths should be optimized (usually under `public/`). Images outside these patterns are not processed. rari also merges `/assets/**` by default for hashed static imports; set `includeBundledAssets: false` to use only the patterns you list.
...
```tsx title="src/components/ResponsiveImage.tsx"
import { Image } from 'rari/image'
...
```tsx title="src/components/CDNImage.tsx"
import { Image } from 'rari/image'
