        ...
        {breadcrumbs.map((crumb, index) => {
          ...
          return (
            <li key={uniqueKey} className="flex items-center">
              {index > 0 && <ChevronRight className="w-4 h-4 mx-2 text-fg-muted" />}
              {isLast || crumb.href == null || crumb.href === '' ? (
                <span className="text-fg-muted">{crumb.label}</span>
              ) : (
                <a href={crumb.href} className="text-fg-secondary hover:text-fg transition-colors">
                  {crumb.label}
                </a>
              )}
            </li>
