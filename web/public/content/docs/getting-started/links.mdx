...
<CodeBlock filename="src/app/dashboard/layout.tsx" language="tsx">{`export default function DashboardLayout({ children }: LayoutProps) {
...
For programmatic navigation in client components, you can use standard browser APIs or trigger link clicks:

<CodeBlock filename="src/app/components/LoginForm.tsx" language="tsx">{`'use client'
...
export default function LoginForm() {
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Navigate using standard browser API
...
The client router will intercept the navigation and handle it as a client-side route change.

### Server Actions with Redirects

Server actions can return redirect responses that are automatically handled:

<CodeBlock filename="src/actions/auth.ts" language="tsx">{`'use server'
...
export async function loginAction(formData: FormData) {
  ...
  // Return redirect in the response
  return { redirect: '/dashboard' }
}
`}</CodeBlock>

When a server action returns a redirect, the client automatically navigates to the specified URL.
