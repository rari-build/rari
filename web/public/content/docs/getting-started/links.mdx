...
If the router hasn't initialized yet (e.g. during SSR or before hydration), `navigate` falls back to `window.location` automatically.
...
<CodeBlock language="tsx">{`import { usePathname, useParams, useSearchParams } from 'rari/router'
...
<CodeBlock language="tsx">{`const router = useRouter()

// Prefetch on hover, pointer enter, etc.
