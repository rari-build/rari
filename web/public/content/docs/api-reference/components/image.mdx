...
- `qualityAllowlist` - Allowed quality values that can be requested (default: `[25, 50, 75, 100]`). The first value is used for preprocessing.
...
**Note:** The quality value must be in the `qualityAllowlist` configured in your `vite.config.ts`. By default, allowed values are `[25, 50, 75, 100]`.
...
- Use `quality={90-100}` for hero images and important visuals
- Use `quality={75}` (default) for most images
- Use `quality={50-60}` for thumbnails and backgrounds
