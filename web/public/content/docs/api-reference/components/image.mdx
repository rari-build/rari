  ...
  plugins: [
    rari({
      images: {
        ...
        formats: ['avif', 'webp'], // Default is ['avif']
...
- `qualityAllowlist` - Allowed quality values that can be requested (default: `[25, 50, 75, 100]`). The first value is used during build-time image preprocessing (manifest generation and preoptimization) to produce optimized source images and placeholders before runtime. Note that this is separate from the runtime `quality` prop default (75), which controls on-demand optimization quality. For example, if `qualityAllowlist: [25, 50, 75, 100]`, build-time preprocessing will use quality 25, while runtime requests default to quality 75 unless specified otherwise.
