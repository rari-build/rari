...
- **Node.js `22.0+`** (<a href="https://nodejs.org/" target="_blank" rel="noopener noreferrer">Active LTS</a> release recommended)
...
With the app router, your application structure is based on the `src/app/` directory. Let's create your first page. See [Routing](/docs/routing) for the full reference on file conventions, dynamic routes, and layouts.
...
rari's Rust runtime handles standard `node_modules` resolution — you can `import` from any npm package the same way you would in Node. This is worth calling out because most Rust-based JS runtimes (like Deno) use URL imports or npm specifiers instead. With rari, your existing `package.json` workflow just works.
...
- [Routing](/docs/routing) — File conventions, dynamic routes, layouts, loading states, error boundaries, and API routes
- [Deploying](/docs/deploying) — Build and deploy to Railway, Render, or any Node.js host
