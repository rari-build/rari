...
- *(module_loader)* Add CommonJS detection and ESM wrapping support
...
- *(module_loader)* Improve error handling in CommonJS detection regex
...
- *(rari)* Remove redundant no-op log message
- *(release)* Remove generated changelog and improve release tooling
- *(release)* Bump rari version to 0.8.6
...
- *(release)* Include pnpm-lock.yaml in release commits
...
- *(deploy)* Improve node version parsing for range specifiers
...
- *(deploy)* Simplify node version comparison logic
- *(deploy)* Improve node version range parsing with compound operators
- *(deploy)* Simplify node version bound extraction logic
- *(cache)* Move cache directory to .cache and improve initialization

### ⚙️ Miscellaneous Tasks

- Remove CodeQL workflow and configuration files
- *(release)* Add LICENSE and README to platform packages
- *(release)* Add file generation for README and LICENSE in packages
- *(rari)* Bump version to 0.8.5
