...
function expectAllLogged(patterns: RegExp[]): void {
  const content = readLog().replace(/\u001B\[[0-9;]*m/g, '')
  const missing = patterns.filter(re => !re.test(content))
  if (missing.length > 0) {
    throw new Error(
      `Missing log patterns in ${LOG_FILE}: ${missing.map(r => r.toString()).join(', ')}\n`
      + `--- last 4KB ---\n${content.slice(-4096)}`,
    )
  }
}
...
test('fetch cache: GET /fetch-test twice - first miss, second hit', async ({ page }) => {
  ...
  const pageCacheHit = /memory cache hit\s+key=\/fetch-test\b/.test(plain)
...
test('handler: multiple GETs against distinct URLs eventually trigger set_with_tags / hit / miss lines', async ({ request }) => {
  ...
  expectAllLogged([/memory cache handler initialized/])
})
