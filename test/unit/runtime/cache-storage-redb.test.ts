...
describe('redbCacheStorage', () => {
  ...
  it('hasRedbOps returns true when both ops are present', async () => {
    const { hasRedbOps } = await import('@rari/use-cache/runtime/cache-storage-redb')
    installRedbOpsMock(makeBackend())
    expect(hasRedbOps()).toBe(true)
  })

  it('hasRedbOps returns false when ops missing', async () => {
    const { hasRedbOps } = await import('@rari/use-cache/runtime/cache-storage-redb')
    uninstallRedbOpsMock()
  ...
  it('read returns null when key is missing', async () => {
    const { createRedbCacheStorage } = await import('@rari/use-cache/runtime/cache-storage-redb')
