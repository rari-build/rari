...
pub const TEST_REDB_PATH: &str = "./.rari/use-cache-redb-test.redb";
...
impl RedbCacheState {
    pub fn from_config() -> Self {
        ...
        let path = remote
            .and_then(|r| match r.handler.as_str() {
                "test" => Some(TEST_REDB_PATH.to_string()),
                _ => r.url.clone(),
            })
            .map(PathBuf::from);
...
fn remove_expired(database: Arc<redb::Database>, key: String) -> Result<(), RedbCacheError> {
    let tx = database.begin_write()?;
    {
        let mut table = tx.open_table(TABLE_DEFINITION)?;
...
{
    tokio::task::spawn_blocking(f).await.map_err(js_error)?.map_err(js_error)
}
...
pub async fn op_redb_cache_get(
    ...
    let redb_state = get_redb_state(state).map_err(js_error)?;
...
pub async fn op_redb_cache_set(
    ...
    let expires_at_ms = expires_at_ms_from_ttl(ttl_ms, default_ttl_secs).map_err(js_error)?;
...
mod tests {
    ...
    fn open_temp_db() -> (tempfile::TempDir, Arc<redb::Database>) {
    ...
    fn write_then_read_roundtrip() {
        ...
        write_to_database(db.clone(), "k1".to_string(), b"hello".to_vec(), EXPIRY_NEVER)
            .expect("write");
        let got = read_from_database(db.clone(), "k1".to_string()).expect("read");
    ...
    fn read_missing_key_returns_none() {
        let (_dir, db) = open_temp_db();
        let got = read_from_database(db.clone(), "absent".to_string()).expect("read");
        assert!(got.is_none());
    }
    ...
    fn expired_entry_returns_none_and_is_removed() {
        ...
        write_to_database(db.clone(), "k".to_string(), b"x".to_vec(), expires_at).expect("write");

        sleep(Duration::from_millis(120));

        let got = read_from_database(db.clone(), "k".to_string()).expect("read");
        assert!(got.is_none());
    ...
    fn never_expires_entry_persists_past_ttl_window() {
        let (_dir, db) = open_temp_db();
        write_to_database(db.clone(), "k".to_string(), b"x".to_vec(), EXPIRY_NEVER).expect("write");
    ...
    fn cleanup_expired_all_removes_only_expired() {
        ...
        .expect("write 1");
        write_to_database(db.clone(), "expired2".to_string(), b"b".to_vec(), now.saturating_sub(5))
            .expect("write 2");
        write_to_database(
            db.clone(),
            "fresh".to_string(),
        ...
        .expect("write 3");
        write_to_database(db.clone(), "forever".to_string(), b"d".to_vec(), EXPIRY_NEVER)
            .expect("write 4");

        let removed = cleanup_expired_all(db.clone()).expect("cleanup");
        assert_eq!(removed, 2);
