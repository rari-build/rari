...
pub fn sanitize_component_output(html: &str, component_id: &str) -> String {
    if html.is_empty() {
    ...
    static WRAPPER_REGEX_CACHE: OnceLock<()> = OnceLock::new();
    WRAPPER_REGEX_CACHE.get_or_init(|| ());

    let wrapper_pattern = format!(
        r#"<div[^>]*?data-component-id=["']{}["'][^>]*?>([\s\S]*?)</div>"#,
        regex::escape(component_id)
    );

    if let Ok(wrapper_regex) = Regex::new(&wrapper_pattern)
        && let Some(captures) = wrapper_regex.captures(&result)
    {
...
mod tests {
    ...
    fn test_sanitize_empty_string() {
        let result = sanitize_component_output("", "test-component");
        assert_eq!(result, "");
    }
    ...
    fn test_sanitize_removes_pre_json() {
        ...
        let result = sanitize_component_output(html, "test-component");
    ...
    fn test_sanitize_with_special_chars_in_component_id() {
        ...
        let result = sanitize_component_output(html, "app/components/test");
        assert_eq!(result, html);
    }
}
