    ...
    fn generate_layout_wrapper(
        ...
        format!(
            r#"
                ...
                const {layout_var} = (layoutResult{index} && layoutResult{index}.type === React.Fragment && layoutResult{index}.props?.children !== undefined)
                    ? layoutResult{index}.props.children
                    : layoutResult{index};
