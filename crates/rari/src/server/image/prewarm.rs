...
pub fn extract_optimize_params_from_html(html: &str) -> Vec<OptimizeParams> {
    ...
    with_blur_placeholder_params(params, &mut seen)
}

fn with_blur_placeholder_params(
...
mod tests {
    ...
    fn extracts_unique_optimize_params_from_html() {
        ...
        let remote_blur = params
            .iter()
            .find(|p| {
                p.url.contains("cdn.example")
                    && p.w == Some(BLUR_PLACEHOLDER_WIDTH)
                    && p.q == BLUR_PLACEHOLDER_QUALITY
            })
            .expect("remote blur");
        assert_eq!(remote_blur.f.as_deref(), Some("jpeg"));
    }
