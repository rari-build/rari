...
impl ImageCache {
    ...
    pub async fn get(&self, key: &str) -> Option<Arc<CachedImage>> {
        ...
        if let Err(e) = self.handler.set(key, read_result, IMG_TTL_SECS).await {
    ...
    pub async fn put(&self, key: String, cached: CachedImage) {
        ...
        if let Err(e) = self.handler.set(&key, serialized, IMG_TTL_SECS).await {
...
mod tests {
    ...
    async fn test_disk_persistence() {
        ...
        let in_handler_b = handler_b.get("persistent").await.unwrap();
