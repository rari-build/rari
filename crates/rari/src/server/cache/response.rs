...
impl ResponseCache {
    ...
    pub async fn set(&self, key: String, response: CachedResponse) {
        ...
        let result = if tags.is_empty() {
            self.handler.set(&key, bytes, ttl).await
        } else {
            self.handler.set_with_tags(&key, bytes, ttl, &tags).await
        };
    ...
    pub async fn update_in_place(&self, key: &str, response: CachedResponse) {
        if !self.config.enabled || self.handler.get(key).await.ok().flatten().is_none() {
            return;
        }

        let bytes = match serde_json::to_vec(&response) {
            Ok(b) => b,
            Err(e) => panic!("CachedResponse must serialize: {e}"),
        };
        let ttl = response.metadata.ttl;
        let tags = response.metadata.tags.clone();
        let result = if tags.is_empty() {
    ...
    pub async fn clear_percentage(&self, percentage: f64) {
        ...
        let keys: Vec<String> = self.handler.get_all_keys();
    ...
    fn resync_entry_count(&self) {
        let live = self.handler.get_all_keys().len();
...
mod tests {
    ...
    impl CacheHandler for StubHandler {
        ...
        async fn set(
            ...
            Ok(SetOutcome { replaced, evicted: 0 })
        }
    ...
    async fn test_new_with_custom_handler() {
        ...
        assert_eq!(*stub.set_calls.lock(), vec!["k".to_string()]);
