...
export function registerPageCacheTags(...tags: readonly string[]): void {
  if (tags.length === 0) return

  const pageCacheTags = (getRariGlobal().pageCacheTags ??= new Set())
  for (const tag of tags) pageCacheTags.add(tag)
}
...
export function addCacheTags(...tags: readonly string[]): void {
  ...
  for (const tag of tags) {
    if (next.size >= 128) {
      console.warn(
        '[rari] cacheTag: maximum of 128 tags per cache scope reached; skipping remaining tags.',
      )
      break
    }
    if (tag.length > 256) {
      console.warn(
        `[rari] cacheTag: tag longer than 256 characters skipped: "${tag.slice(0, 32)}..."`,
      )
      continue
    }
    next.add(tag)
