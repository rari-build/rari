...
- *(vite)* enhance client reference handling and inline server actions by @skiniks
- *(vite)* improve inline server action handling and regex updates by @skiniks
- *(vite)* add export name collection and enhance import handling by @skiniks
- *(vite)* enhance export name collection and improve inline server action registration by @skiniks
- *(vite)* enhance handling of type-only imports and improve inline server action registration by @skiniks

### 🐛 Bug Fixes

- *(rsc)* fail loudly on flight client patch drift and stamp vendor versions by @skiniks
- *(vite)* replace regex client-import rewrites with lexer spans by @skiniks

### 🚜 Refactor

- *(server-port)* streamline server port resolution and enhance HMR dependency tracking by @skiniks

### ⚡ Performance

- *(vite)* scope HMR re-registration to affected components by @skiniks

### ⚙️ Miscellaneous Tasks

- *(ci)* add React ESM bundling step to CI workflow and improve error handling in flight browser client patch by @skiniks
- *(dependencies)* update various package versions by @skiniks
- *(rari)* bump version to 0.15.8 in Cargo.toml by @skiniks
- *(dependencies)* update @rari/use-cache and related packages to version 0.15.8 by @skiniks
